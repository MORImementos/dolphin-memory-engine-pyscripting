...
dme_list = [
    'MemWatch', '_dolphin_memory_engine', 'assert_hooked', 'follow_pointers', 'read_byte', 'read_bytes', 'read_double', 'read_float', 'write_byte', 'write_bytes', 'write_double', 'write_float', 'write_word']
print(dme.assert_hooked())
...
def write_double(addr, value):
    hook()
    dme.write_doule(addr, value)


def write_vec3_sequential(addr, value: Vector3):
    hook()
...
def write_vec3_bindat(addr, v: Vector3) -> None:
    hook()
...
class DolphinWord():
    ...
    def read(self, index=0) -> int:
        # revised to check for signed / unsigned
        unsigned_value = read_word(self._addr + index * self._struct_size)
        return unsigned_value if unsigned_value < 0x80000000 else unsigned_value - 0x100000000
...
class DolphinVec3:
    ...
    def write(self, v: Vector3, index=0) -> None:
        write_vec3_sequential(self._addr + index * self._struct_size, v)
